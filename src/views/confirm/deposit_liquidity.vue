      ...
      <section className='cards'>
        <div className='card'>
          ...
          <div className='simple-card' style="background: white;">
            ...
            <div v-if="error_token_approve2 == true" className='wrapper2i' style="margin-bottom: 1rem;">
              <p style="text-align: left;">Approve {{token_symbol2}}</p>
              <p style="text-align: right; color: rgb(223, 49, 49);">Failed</p>
            </div> 
            <div v-if="start_to_fifty2 == false && fifty_to_one_hundred1 == false && error_token_approve2 == false" className='wrapper2i' style="margin-bottom: 1rem;"> 
              <p style="text-align: left;">Approve</p>
              <p style="text-align: right;">{{token_symbol2}}</p>
            </div>  
            <div v-if="start_to_fifty2 == false && fifty_to_one_hundred1 == true && error_token_approve2 == false" className='wrapper2i' style="margin-bottom: 1rem;"> 
              <p style="text-align: left;">Approve {{token_symbol2}}</p>
              <div style="text-align: right;">Sign Tx<span class="one">.</span><span class="two">.</span><span class="three">.</span></div>
            </div>       
            <div v-if="start_to_fifty2 == true && fifty_to_one_hundred2 == false" className='wrapper2i' style="margin-bottom: 1rem;">
              <p style="text-align: left;">Approve {{token_symbol2}}</p>
              <div @click="open_hash(hash_token_approve2)" style="text-align: right; cursor: pointer;">Tx Pending<span class="one">.</span><span class="two">.</span><span class="three">.</span></div>
            </div>          
            <div v-if="fifty_to_one_hundred2 == true" className='wrapper2i' style="margin-bottom: 1rem;">
              <p style="text-align: left;">Approve {{token_symbol2}}</p>
              <p @click="open_hash(hash_token_approve2)" style="text-align: right; cursor: pointer; color: #008a7b;">Tx Approved</p>
            </div>                   
            <div style="margin-bottom: 1rem;" id="modules">
              <div class="module"><p style="background: #595ec1; color: white">0.0%</p></div>
              <div class="module"><p :style="[start_to_fifty2===true ? {'background': '#595ec1', 'color': 'white' }: {'background': '#e3f4fe', 'color': 'black'}]">20%</p></div>
              <div class="module"><p :style="[start_to_fifty2===true ? {'background': '#595ec1', 'color': 'white' }: {'background': '#e3f4fe', 'color': 'black'}]">40%</p></div>
              <div class="module"><p :style="[start_to_fifty2===true ? {'background': '#595ec1', 'color': 'white' }: {'background': '#e3f4fe', 'color': 'black'}]">60%</p></div>
              <div class="module"><p :style="[fifty_to_one_hundred2===true ? {'background': '#595ec1', 'color': 'white' }: {'background': '#e3f4fe', 'color': 'black'}]">80%</p></div>
              <div class="module"><p :style="[fifty_to_one_hundred2===true ? {'background': '#595ec1', 'color': 'white' }: {'background': '#e3f4fe', 'color': 'black'}]">100%</p></div>
              <div class="module"><p :style="[fifty_to_one_hundred2===true ? {'background': '#595ec1', 'color': 'white' }: {'background': '#e3f4fe', 'color': 'black'}]">DONE</p></div>            
            </div>
            <div v-if="error_sign_liquidity == true" className='wrapper2i' style="margin-bottom: 1rem;">
              <h4 style="text-align: left;">Deposit {{token_symbol1 + ' & ' + token_symbol2}}</h4>
              <p style="text-align: right; color: rgb(223, 49, 49);">Failed</p>
            </div> 
            <div v-if="tx_confirm1 == false && fifty_to_one_hundred2 == false && error_sign_liquidity == false" className='wrapper2i' style="margin-bottom: 1rem;">
              <h4 style="text-align: left;">Deposit</h4>
              <p style="text-align: right;">{{token_symbol1 + ' & ' + token_symbol2}}</p>
            </div> 
            <div v-if="tx_confirm1 == false && fifty_to_one_hundred2 == true && error_sign_liquidity == false" className='wrapper2i' style="margin-bottom: 1rem;">
              <h4 style="text-align: left;">Deposit {{token_symbol1 + ' & ' + token_symbol2}}</h4>
              <div style="text-align: right;">Sign Tx<span class="one">.</span><span class="two">.</span><span class="three">.</span></div>
            </div> 
            ...
            <div style="margin-bottom: 1rem; grid-template-columns: 0.5fr 2.6fr;" className='wrapper2a'>
              ...
              <p style="text-align: right;">{{token_price1.toString().substring(0, 12)}} <mark class="purple">{{token_symbol1}}</mark></p>
            </div>
            <div style="margin-bottom: 1rem; grid-template-columns: 0.5fr 2.6fr;" className='wrapper2a'>
              <p style="text-align: left;">Token 2</p>
              <p style="text-align: right;">{{token_price2.toString().substring(0, 12)}} <mark class="purple">{{token_symbol2}}</mark></p>
            </div>
            <div className='wrapper2a'>
              <h4 style="text-align: left;">Receive</h4>
              <p style="text-align: right;">{{lp_amount.toString().substring(0, 12)}} <mark class="purple">{{'xDx' + token_symbol1 + 'x' + token_symbol2}}</mark></p>
            </div>  
            ...
            <p v-if="tx_confirm2 == true" style="text-align: center; margin-top: 1rem; color: #008a7b;">Thank you for your support!</p>
            ...
            <p v-if="error_token_approve2 == true" style="text-align: center; margin-top: 1rem; color: rgb(223, 49, 49);">Approve {{token_symbol2}}: You denied tx signature!</p>
            <p v-if="error_sign_liquidity == true" style="text-align: center; margin-top: 1rem; color: rgb(223, 49, 49);">New Liquidity {{token_symbol1}}/{{token_symbol2}}: Tx Failed!</p>
          </div>
        </div>
...
export default {
  ...
  setup() {
    ...
    const hash_token_approve2 = computed(() => store.state.token_approve_hash2);
    const hash_deposit_approve = computed(() => store.state.add_liquidity_hash);
    ...
    const start_to_fifty2 = computed(() => store.state.start_to_fifty2);
    const fifty_to_one_hundred1 = computed(() => store.state.fifty_to_one_hundred1);
    const fifty_to_one_hundred2 = computed(() => store.state.fifty_to_one_hundred2);
    // errors
    const error_sign_liquidity = computed(() => store.state.add_liquidity_sign_tx);
    const error_token_approve1 = computed(() => store.state.error_token_approve1);
    const error_token_approve2 = computed(() => store.state.error_token_approve2);
    // remove liquidity info
    const lp_amount = computed(() => store.state.lp_amount); // LP receive
    const token_price1 = computed(() => store.state.temp_price1); // amount token in 1
    const token_price2 = computed(() => store.state.temp_price2); // amount token in 2
    ...
    return {
      ...
      fifty_to_one_hundred2,
      // errors
      error_token_approve1,
      error_token_approve2,
...
.wrapper2 {
  display: grid;
  grid-template-columns: 0.25fr 2.6fr;
  grid-column-gap: 5px;
  grid-row-gap: 1em;
}
