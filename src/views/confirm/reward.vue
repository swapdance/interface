      ...
      <section className='cards'>
        <div className='card'>
          ...
          <div className='simple-card' style="background: white;">
            ...
            <div v-if="error_sign_get_reward == true" className='wrapper2i' style="margin-bottom: 1rem;">
              <h4 style="text-align: left;">Sign Tx</h4>
              <p style="text-align: right; color: rgb(223, 49, 49);">Failed</p>
            </div> 
            <div v-if="tx_confirm1 == false && error_sign_get_reward == false" className='wrapper2i' style="margin-bottom: 1rem;">
            ...
            <div v-if="tx_confirm1 == true && tx_confirm2 == false && error_sign_get_reward == false" className='wrapper2i' style="margin-bottom: 1rem;">
              <p style="text-align: left;">Tx Progress</p>
              <div @click="open_hash(hash_approve_reward)" style="text-align: right; cursor: pointer;">Tx Pending<span class="one">.</span><span class="two">.</span><span class="three">.</span></div>
            </div>          
            <div v-if="tx_confirm2 == true" className='wrapper2i' style="margin-bottom: 1rem;">
              <p style="text-align: left;">Tx Progress</p>
              <p @click="open_hash(hash_approve_reward)" style="text-align: right; cursor: pointer; color: #008a7b;">Tx Approved</p>
            </div>              
            ...
            <div className='wrapper2a'>
              ...
              <p style="text-align: right;">{{available_reward}} <mark class="purple">DANCE</mark></p>
            </div>  
            <div v-if="tx_confirm2 == true" className='wrapper2' style="margin-bottom: 1rem; margin-top: 1rem; grid-template-columns: 1.5fr 1.5fr;">
              ...
              <p @click="open_hash(hash_approve_reward)" style="text-align: right; color: #008a7b; cursor: pointer;">{{hash_approve_reward.toString().substring(0, 18) + '...'}}</p>
            </div>  
            <p v-if="tx_confirm2 == true" style="text-align: center; margin-top: 1rem; color: #008a7b;">Amazing! You got the trophy!</p>
            <p v-if="error_sign_get_reward == true" style="text-align: center; margin-top: 1rem; color: rgb(223, 49, 49);">Withdraw <mark class="purple">DANCE</mark>: Tx Failed!</p>
          </div>
        </div>
...
export default {
  ...
  setup() {
    ...
    const hash_approve_reward = computed(() => store.state.withdraw_pot_reward_hash);
    
    // new pair info
    const available_reward = computed(() => store.state.lp_amount); // without round reward
    // errors
    const error_sign_get_reward = computed(() => store.state.error_sign_get_reward);
