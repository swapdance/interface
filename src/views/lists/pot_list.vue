...
export default {
  ...
  setup() {
    ...
    useMeta({
      title: 'Proof of Trade Pool List: Swap.Dance', 
    ...
    const proof_of_trade_page = computed(() => store.state.proof_of_trade_page);
    ...
    function select_go_back(pair_station) {
      ...
      if (proof_of_trade_page.value == "1"){
        store.state.stake_proof_of_trade_station = pair_station;
        store.dispatch("change_pot_stake_station");
      } else if (proof_of_trade_page.value == "2"){
        store.state.unstake_proof_of_trade_station = pair_station;
        store.dispatch("change_pot_unstake_station");
      } else if (proof_of_trade_page.value == "3") {
        store.state.get_reward_proof_of_trade_station = pair_station;
        store.dispatch("change_pot_get_reward_station");
      }
      history.back();
    }
