      ...
      <section className='cards'>
        <div className='card'>
          ...
          <div className='simple-card'>
            ...
            <div v-if="add_liquidity_station_type == 1 && token_balance_calc1 == 0 && token_balance_calc2 == 0" className='wrapper3plus'>
              <router-link @click="check()" :to="{name: 'Pair List'}">
                <input type="sell_select_tokens" style="text-align: left; width: 100%;" :value="token_symbol1 + '/' + token_symbol2 + ' ⩖'" :disabled="true">
              </router-link>
              <input style="text-right: left; width: 100%;" id="new_liquidity_input_double" :value="token_amount1" @input="token_amount1 = $event.target.value" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" placeholder="1.0">
              <input style="text-right: left; width: 100%;" id="new_liquidity_input_double2" :value="token_amount_new_liquidity2" @input="token_amount_new_liquidity2 = $event.target.value" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" placeholder="1.0">
            </div>
            ...
            <div v-if="add_liquidity_station_type == 1 && token_balance_calc1 == 0 && token_balance_calc2 == 0"  className='wrapper2' style="grid-template-columns: 0.65fr 2.45fr;">
              <p style="text-align: left;">Set Prices</p>
          ...
          <div className='simple-card' style="background: white">
            ...
            <div className='wrapper2a' style="margin-bottom: 1rem; grid-template-columns: 0.5fr 2.6fr;">
              ...
              <p style="text-align: right;">{{token_amount1.toString().substring(0, 12)}} <mark class="purple">{{token_symbol1}}</mark></p>
            </div>
            <div v-if="token_balance_calc1 > 0 && token_balance_calc2 > 0 || (add_liquidity_station_type == 0 && token_balance_calc1 == 0 && token_balance_calc2 == 0)" className='wrapper2a' style="margin-bottom: 1rem; grid-template-columns: 0.55fr 2.6fr;">
              ...
              <p style="text-align: right;">{{token_amount2.toString().substring(0, 12)}} <mark class="purple">{{token_symbol2}}</mark></p>
            </div>
            <div v-if="add_liquidity_station_type == 1 && token_balance_calc1 == 0 && token_balance_calc2 == 0" className='wrapper2a' style="margin-bottom: 1rem; grid-template-columns: 0.5fr 2.6fr;">
              <p style="text-align: left;">Token 2</p>
              <p style="text-align: right;">{{token_amount_new_liquidity2.toString().substring(0, 12)}} <mark class="purple">{{token_symbol2}}</mark></p>
            </div>
            <div className='wrapper2inline'  style="margin-bottom: 1rem; grid-template-columns: 0.3fr 2.7fr;">
              <p style="text-align: left;">Receive</p>
              <p style="text-align: right;">{{lp_received}} <mark class="purple">LP</mark></p>
            </div>
...
export default {
  ...
  methods: {
    ...
    async liquidity_connect() {
      ...
      //  let a = BigNumber.from(ENumber(amount1).toString());
      //  let b = BigNumber.from(ENumber(amount2).toString());
        let a = ethers.utils.parseUnits(ENumber(amount1).toString(), this.token_decimal1);
        let b = ethers.utils.parseUnits(ENumber(amount2).toString(), this.token_decimal2);
        let user_token_balance1 = ethers.utils.formatUnits(a, this.token_decimal1);
        let user_token_balance2 = ethers.utils.formatUnits(b, this.token_decimal2);
        
        let check_max_amount1 = ethers.utils.parseUnits(ENumber(max_amount1).toString(), this.token_decimal1);
        let check_max_amount2 = ethers.utils.parseUnits(ENumber(max_amount2).toString(), this.token_decimal2);
        ...
        if (Number(max_amount1) == 0){
          ...
          this.error_amount_token2 = false;
          this.error_amount_token1 = false;
        } else if (Number(user_token_balance1) > Number(check_max_amount1)) {
          this.error_set_amount_token1 = false;
          this.error_set_amount_token2 = false;
          this.error_amount_token1 = false;
          this.error_amount_token2 = false;
          this.error_max_amount_token1 = true;
        } else if (Number(user_token_balance2) > Number(check_max_amount2)) {
          this.error_amount_token1 = false;
          this.error_amount_token2 = false;
          this.error_max_amount_token1 = false;
        ...
        //  console.log("before", pair_station, amount1.toString(), amount_slippage1.toString(), amount2.toString(), amount_slippage2.toString(), expiry.toString())
          if (this.token_balance1 == 0 && this.token_balance2 == 0){
